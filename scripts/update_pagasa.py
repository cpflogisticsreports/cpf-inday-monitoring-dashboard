...
def parse_bulletin(text, storm):
    ...
    mv = re.search(
        r"moving\s+([A-Za-z\- ]+?)\s+at\s+(?:the speed of\s+)?([\d]+)\s*(?:km/?h|kph|kilometers per hour)",
        section, re.I)
...
def main():
    ...
    try:
        raw = open(args.from_file, encoding="utf-8", errors="ignore").read() if args.from_file \
              else fetch_html(PAGASA_URL)
    except Exception as e:
        print("::warning::Could not fetch PAGASA page (%s). Keeping last verified file." % e)
        return 0
